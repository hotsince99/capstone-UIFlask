...
def index():
# %% copy df2 from playstore
    df2 = playstore.copy()

    # %% Statistik
    # %% Dataframe top_category dibuat untuk menyimpan frekuensi aplikasi untuk setiap Category. 
    # %% Gunakan crosstab untuk menghitung frekuensi aplikasi di setiap category kemudian gunakan 'Jumlah' # %% == 'banyaknya'
...
# %% dictionary most_category, total, rev_table
    ...
    stats = {
        ...
        'rev_table' : df2.groupby(by= ['Category','App'])\
...
# %%
    ...
    ## Scatter Plot punya josua, membandingkan Reviews dan price

    # Ukuran scatter menggambarkan berapa banyak pengguna yang telah menginstall aplikasi 
    X = df2['Reviews'].values # %% axis x
    Y = df2['Price'].values # %% axis y
    area = playstore['Installs'].values/10000000 # %% ukuran besar/kecilnya lingkaran scatter plot
    fig = plt.figure(figsize=(5,5))
    fig.add_subplot()
    # isi nama method untuk scatter plot, variabel x, dan variabel y
    ...
    plt.xlabel('Reviews')
    plt.ylabel('Price')
    plt.savefig('rev_price.png',bbox_inches="tight")
...
# %% comment ini ditulis untuk membatasi run cell result4

    # Tambahkan hasil result plot pada fungsi render_template() #tambahin result4
